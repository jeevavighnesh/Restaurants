...
SET @NUMBEROFINPUTORDERS = (SELECT COUNT(*) FROM INPUTORDERS WHERE SEATNUMBER = SEATID);
...
THEN
	...
	THEN
		IF (SELECT COUNT(DISTINCT FOOD) FROM INPUTORDERS WHERE SEATNUMBER = SEATID) <= @ITEMLIMIT
		THEN
WHILELOOP:	WHILE (SELECT ID FROM INPUTORDERS WHERE ID = @SUBORDERID AND SEATNUMBER = SEATNUMBER) IS NOT NULL
		DO
