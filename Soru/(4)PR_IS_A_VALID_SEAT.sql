...
BEGIN
	...
	ELSEIF (SELECT STATUSID FROM SEAT WHERE ID = SEATNUMBER) = 2 OR (SELECT CONCURRENCY FROM SEAT WHERE ID = SEATNUMBER) = TRUE -- SEAT TAKEN
	THEN
		SET COMMENTS = "THE CHOSEN SEAT HAS ALREADY BEEN TAKEN YOU COULD WAIT FOR IT OR ASK FOR ANOTHER ONE";
	ELSE
		SET ERROR = FALSE;
		UPDATE SEAT SET CONCURRENCY = NOT CONCURRENCY WHERE ID = SEATNUMBER; 
	END IF;
END !
